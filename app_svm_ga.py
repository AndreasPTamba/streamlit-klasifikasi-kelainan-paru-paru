...
st.markdown('''
    <style>
    .main-title { text-align: center; color: #2c3e50; font-size: 2.5em; font-weight: bold; margin-bottom: 5px; }
    .sub-title { text-align: center; color: #7f8c8d; font-size: 1.2em; margin-bottom: 30px; }
    .stButton>button { width: 100%; background-color: #3498db; color: white; border-radius: 5px; padding: 10px; font-size: 1.1em; border: none; }
    .stButton>button:hover { background-color: #2980b9; }
    .result-box-abnormal { background-color: #ffeaea; border: 2px solid #ff4d4d; border-radius: 10px; padding: 20px; text-align: center; margin-top: 20px; }
    .result-text-abnormal { color: #cc0000; font-size: 1.8em; font-weight: bold; margin: 0; }
    .result-box-normal { background-color: #eafaf1; border: 2px solid #2ecc71; border-radius: 10px; padding: 20px; text-align: center; margin-top: 20px; }
    .result-text-normal { color: #27ae60; font-size: 1.8em; font-weight: bold; margin: 0; }
    .confidence-text { color: #555; font-size: 1em; margin-top: 10px; }
    </style>
...
def extract_features(image):
    # Tahap 1: Grayscale -> TV-Chambolle -> EqualizeHist
    ...
    # Tahap 2: Resize (240x240) -> ToTensor -> Normalize
    ...
    feature_vector = []
    coeffs = pywt.wavedec2(final_img, 'haar', level=1)
    cA = coeffs[0]
    cH, cV, cD = coeffs[1]
    ...
    for subband_name, subband in coefficients.items():
        ...
        feature_vector.extend([
            np.mean(c), np.var(c), np.sum(c**2) / len(c),
            calculate_entropy(c), skew(c), kurtosis(c),
            np.max(c), np.min(c)
        ])
...
if uploaded_file is not None:
    ...
    if st.button("🔍 Analisis Citra"):
        if svm_model is None:
            st.error("❌ Model SVM-GA tidak ditemukan.")
        else:
            with st.spinner('Memproses gambar dan mengekstraksi fitur DWT...'):
                try:
                    ...
                    # 4. Hitung Confidence untuk filter gambar aneh
                    confidence = 0.0
                    try:
                        ...
                        if len(decision_scores.shape) > 1 or len(decision_scores) > 1:
                    ...
                    if confidence < CONFIDENCE_THRESHOLD:
                        st.warning("⚠️ **Peringatan:** Gambar kemungkinan bukan citra Chest X-Ray yang valid atau kualitasnya terlalu buruk.")
                        
                    # Selalu tampilkan prediksi
                    if True:
                        class_labels = {
                            0: "COVID-19",
                            1: "Normal",
                        ...
                        # Tampilan UI tanpa angka akurasi
                        st.markdown(f'''
                            <div class="{box_class}">
